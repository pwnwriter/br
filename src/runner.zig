...
fn workerScript(gpa: Allocator, io: Io, name: []const u8) ![]const u8 {
    return std.fs.path.join(gpa, &.{ try workerDir(gpa, io), name });
}

/// Locates the `worker/` directory that holds the Bun scripts, without relying
/// on the current working directory. Order:
///   1. $BR_WORKER_DIR               — explicit override
///   2. <exe dir>/worker             — release tarball (br sits next to worker/)
///   3. <exe dir>/../share/br/worker — packaged install (bin/ + share/)
///   4. "worker"                     — dev fallback (running from the repo root)
fn workerDir(gpa: Allocator, io: Io) ![]const u8 {
